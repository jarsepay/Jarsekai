...
export async function handler(chatUpdate) {
    ...
    try {
        ...
        const ___dirname = path.join(path.dirname(fileURLToPath(import.meta.url)), './Jarsekai')
        for (let name in global.jarspy) {
            let plugin = global.jarspy[name]
            if (!plugin)
                continue
            if (plugin.disabled)
                continue
...
export async function participantsUpdate({ id, participants, action }) {
        ...
        switch (action) {
            ...
            case 'remove':
             ...
             forwardingScore: 0,
              externalAdReply: {
                title: action === 'add' ? (await style(`Selamat datang di ${gpname}\nMember ke: ${member}`, 1)) : (await style(`${name} baru saja keluar dari ${gpname}\nSisa member: ${member}`, 1)),
                body: (await style(`Member count: ${member}`, 1)),
              ...
              break                          
      case 'promote':
            text = (chat.sPromote || this.spromote || conn.spromote || '👑 @user ```telah di promosikan menjadi Admin```')
        case 'demote':
            if (!text)
                text = (chat.sDemote || this.sdemote || conn.sdemote || '👑 @user ```telah di demote dari Admin```')
